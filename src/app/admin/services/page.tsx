                ...
                <tr key={s.id} className={`border-b border-white/5 hover:bg-white/3 transition-colors ${i === rows.length - 1 ? 'border-0' : ''}`}>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-1">
                      ...
                      <button type="button" onClick={() => startEdit(s)} title="Edit"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="relative w-full max-w-lg bg-linear-to-br from-slate-900 to-slate-800 border border-white/8 rounded-2xl p-7 overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-white font-bold text-lg">{editing ? 'Edit Service' : 'Add Service'}</h2>
              <button type="button" onClick={() => { setOpen(false); setEditing(null); setForm(EMPTY); }} title="Close" className="text-slate-400 hover:text-white transition-colors"><X className="w-5 h-5" /></button>
            </div>
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-semibold uppercase tracking-widest text-slate-400">Features</label>
                <textarea rows={4} placeholder="One feature per line..."
                  className="bg-slate-800/60 border border-white/8 focus:border-red-500/40 text-white placeholder-slate-500 rounded-xl px-4 py-2.5 text-sm outline-none transition-colors resize-none"
